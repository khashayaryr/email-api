...
# --- Boilerplate and Data Loading (No changes) ---
...
# --- Session State and Helper Functions (No changes) ---
...
def generate_preview(body_template, recipient_profile, sender_profile, include_signature):
    preview_text = body_template; sender_data = {f"my_{key}": value for key, value in sender_profile.items()}; all_placeholders = {**recipient_profile, **sender_data}
    for key, value in all_placeholders.items(): preview_text = preview_text.replace(f"{{{key}}}", str(value))
    if include_signature: signature = sender_profile.get("signature", ""); preview_text = f"{preview_text}\n\n--\n{signature}"
    return preview_text
...
def on_template_change():
    ...
    if selected_template_name != "-- Write from scratch --":
        template = template_map[selected_template_name]; st.session_state.email_subject = template.get("subject", ""); st.session_state.email_body = template.get("body", "")
    else:
...
with col2:
    ...
    if not selected_profile_names: st.info("Select one or more recipients to see a live preview.")
    else:
        ...
        for i, tab in enumerate(recipient_tabs):
            with tab:
                current_recipient_name = selected_profile_names[i]; current_recipient_profile = profile_map_by_name[current_recipient_name]
                preview_content = generate_preview(st.session_state.email_body, current_recipient_profile, my_profile, add_signature)
                st.markdown(f"**To:** {current_recipient_profile['name']} <{current_recipient_profile['email']}>"); st.markdown(f"**Subject:** {st.session_state.email_subject}"); st.divider()
                with st.container(border=True, height=450): st.text(preview_content)
...
# FIX: The schedule date picker is now outside the form to make its value available
...
if set_reminder_toggle:
    # FIX: The validation now correctly uses the value from the date picker above
...
with st.form("action_form"):
    ...
    # Only the time picker and submit buttons are in the form
    ...
    if schedule_button or send_now_button:
        subject = st.session_state.email_subject; body = st.session_state.email_body
        if not selected_profile_names or not subject or not body:
            ...
            if uploaded_files:
                for uploaded_file in uploaded_files:
                    unique_filename = f"{uuid.uuid4()}_{uploaded_file.name}"; save_path = os.path.join(ATTACHMENTS_DIR, unique_filename)
            ...
            if send_now_button:
                schedule_datetime = datetime.datetime.now(); action_text = "queued for immediate sending"
            else:
                # FIX: Combine the date from outside the form with the time from inside
            ...
            if reminder_to_save: success_message += f" with a reminder set for {reminder_to_save.strftime('%Y-%m-%d')}."
